...
export default function Bots() {
  ...
  const handleStart = (id) => {
    updateBot.mutate({ id, data: { status: "active" } });
    toast.success("Bot iniciado");
  };
  const handlePause = (id) => {
    updateBot.mutate({ id, data: { status: "paused" } });
    toast.info("Bot pausado");
  };
  const handleStop = (id) => {
    updateBot.mutate({ id, data: { status: "stopped" } });
    toast.error("Bot detenido");
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">4 bots independientes — gestión conservadora de riesgo</p>
      </div>
